...
compt = 0
id.each do |id|
    if id.include?("aude") || id.include?("Aude")
    compt += 1
    end
end
puts compt 
...
puts id.sort_by {|id| id.downcase}


tab = id.sort_by {|fifty| fifty.length}
puts tab [0,49]

position = id.index ("@epenser")
puts position
